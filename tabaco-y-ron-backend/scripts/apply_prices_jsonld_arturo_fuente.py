"""Asigna precio a los productos Arturo Fuente sin precio usando el JSON-LD canónico.

Cada archivo HTML embebe un bloque `<script type="application/ld+json">` con un
nodo Product que contiene `name` + offers + price. Esa es la fuente canónica
para el producto al que pertenece la página (no se mezcla con relacionados).
...
Uso:
    python -m scripts.apply_prices_jsonld_arturo_fuente
"""
